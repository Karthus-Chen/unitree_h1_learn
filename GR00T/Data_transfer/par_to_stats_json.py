...
# # 目录设置
# parquet_dir = "/home/wwt/N1.5_data/par"
# stats_json_path = "/home/wwt/N1.5_data/stats.json"
# output_stats_json_path = "/home/wwt/N1.5_data/par"

# # 指定统计数据的顺序
# expected_keys = [
#     "observation.state", "action", "timestamp", "annotation.human.action.task_description",
#     "task_index", "annotation.human.validity", "episode_index", "index",
#     "next.reward", "next.done"
# ]

# # 读取所有 parquet 文件
# parquet_files = sorted([f for f in os.listdir(parquet_dir) if f.startswith("episode_") and f.endswith(".parquet")])

# # 存储所有数据
# data_dict = {key: [] for key in expected_keys}
...
#                 data_dict[column].extend(df[column].apply(lambda x: np.array(x)))  # 转换为 NumPy 数组
#             else:
#                 data_dict[column].extend(df[column].values)

# # 计算统计信息
# stats = {}
# for key in expected_keys:
#     values = np.array(data_dict.get(key, []))  # 获取数据，若无则为空数组

#     if values.size == 0:  # 处理缺失字段
#         stats[key] = {
#             "mean": [None],
#             "std": [None],
#             "min": [None],
...
# # 读取原始 stats.json 以保持格式
# with open(stats_json_path, "r") as f:
#     original_stats = json.load(f)

# # 更新 stats.json 结构，并按指定顺序排序
# final_stats = {key: stats.get(key, original_stats.get(key, {})) for key in expected_keys}


# output_dir = os.path.dirname(output_stats_json_path)
...
# print(f"✅ 统计数据已生成: {output_stats_json_path}")
